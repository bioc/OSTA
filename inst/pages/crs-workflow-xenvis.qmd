...
In general, it is possible to bin at the transcript-level (subcellular) or cell-level.
...
```{r plt-sum, warning=FALSE, fig.width=7, fig.height=3}
...
```{r plt-clu, collapse=TRUE, message=FALSE, fig.width=4, fig.height=2.5}
...
For further literature and resources on 
cross-modal analyses, we refer readers to:

- @sec-crs-registration on registration,
- @sec-crs-imputation on imputation, and
- [Beyond this book](#sec-beyond) on integration and multi-modality.
