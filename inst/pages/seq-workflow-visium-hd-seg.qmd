...
Analogous to other chapters, we start out by retrieving data files from the OSF repository using `r BiocStyle::Biocpkg("OSTA.data")`. Output files from Space Ranger v4 are located under `/segmented_outputs` (see @sec-bkg-importing-data for details on Visium HD output structure).
...
`Space Ranger` also provides information on the mapping between binned outputs
(2, 8, and 16 µm) and segmented cells. These are stored as cell metadata slot `map`:
...
Quick inspection shows us the number of 2 µm bins that were segmented,
either as part of nuclei or whole cells:

```{r check-bin-cell-mapping}
# number of segmented 2 µm bins
summary(sapply(spe$map, nrow))
# fraction of 2 µm bins assigned to nucleus
summary(sapply(spe$map, \(.) mean(.$in_nucleus)))
```

For runtime reasons, we will crop the tissue
...
```{r vhdseg-plot-roi, fig.width=3, fig.height=3}
...
We can estimate cell areas based on the number of 2 µm bins covered by each mask, e.g., 10 bins (4 µm^2^ each) would correspond to an area of 40 µm^2^. Based on this, we can also approximate the area of each cell's nucleus using the `in_nucleus` flag provided in the bin-to-cell mapping information.

```{r um2}
# get cell areas = 4x number of 2 µm bins
sfe$um2 <- 4*sapply(sfe$map, nrow)
# get fraction of 2 µm bins that are nuclear
...
that are contained in 8 µm and 16 µm bins, respectively:
...
```{r vhdseg-bin-cell-summary, echo=FALSE}
max <- sapply(ncs, max)
med <- sapply(ncs, median)
q75 <- sapply(ncs, quantile, 0.75)
```

Here, we can observe that 8 µm/16 µm bins map to a median of `r med[1]`/`r med[2]`
...
We can calculate standard cell-level quality control metrics using
...
```{r vhdseg-plot-qc-spatial, message=FALSE, fig.width=8, fig.height=4}
#| code-fold: true
...
ol <- as.matrix(cd[ol])
# percentage of cells excluded for each
...
```{r vhdseg-plot-cell-area, fig.width=5, fig.height=3.5}
#| code-fold: true
# wrangling
