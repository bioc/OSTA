# Example datasets {#sec-bkg-example-datasets}
...
These datasets have been deposited in an Open Storage Framework (OSF) repository [here](https://osf.io/5n4q3), and can be easily queried and downloaded using functions from the `r BiocStyle::CRANpkg("osfr")` CRAN package. For convenience, we have implemented the `r BiocStyle::Biocpkg("OSTA.data")` Bioconductor package to:

::: {.column-margin}
:::: {.callout-note collapse="false" title="cache invalidation"}
It can happen that we change, add, or remove data from the OSF repository underlying `OSTA.data`. Should you ever run into any issues that might be related to this, we suggest removing affected cache resource(s) as follows, and retrieving these data anew.
...
In addition, several datasets are available from the `r BiocStyle::Biocpkg("STexampleData")` Bioconductor package as pre-formatted `SpatialExperiment` and `SingleCellExperiment` formats. These data objects are stored on Bioconductor's `r BiocStyle::Biocpkg("ExperimentHub")` resource, and can be loaded in R by querying `ExperimentHub` or using loader functions provided in the `STexampleData` package.

```{r list-stexampledata, message=FALSE, warning=FALSE}
...
- reference: @deOliveira2025-high-def
