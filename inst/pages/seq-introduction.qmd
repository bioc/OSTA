...
The remainder of this part follows that analysis path: @sec-seq-reads-to-counts describes the conversion from reads to count matrices, @sec-bkg-importing-data covers reading processed platform outputs into R/Bioconductor objects, and @sec-seq-quality-control and @sec-seq-intermediate-processing demonstrate early count-level analysis steps before the workflow chapters.
...
```{r fig-visium-platform, echo=FALSE, out.width="70%", fig.cap="Schematic illustrating the 10x Genomics Visium platform. Source: [10x Genomics Visium](https://www.10xgenomics.com/spatial-transcriptomics/)"}
...
An earlier version of the platform was originally developed by @Stahl2016-ST, which was subsequently improved and commercialized as the 10x Genomics Visium platform.
...
The [10x Genomics Visium HD](https://www.10xgenomics.com/products/visium-hd-spatial-gene-expression) platform provides higher spatial resolution. Capture area dimensions are 6.5 x 6.5 mm, with a continuous "lawn" of capture oligonucleotides arranged into a grid of 2 x 2 µm barcoded squares, which can be aggregated into 8 x 8 µm and 16 x 16 µm bins, or cells based on cell boundary polygon coordinates (e.g. using methods such as `Bin2cell` [@Polanski2024-bin2cell]) for downstream analyses. There are no gaps between the grid squares, and there are two capture areas per slide.
...
A paper describing the platform and its application to human colorectal cancer samples is available from 10x Genomics [@deOliveira2025-high-def].
...
The [Curio Seeker](https://curiobioscience.com/product/) platform is a commercially available implementation and extension of the Slide-seqV2 platform originally developed by @Stickels2021-Slide-seqV2.
...
The [STOmics Stereo-seq](https://en.stomics.tech/why-stereo-seq/index.html) platform provides a large field of view and high spatial resolution. Chip sizes are currently available in either 5 mm x 5 mm or 10 mm x 10 mm sizes, with customized larger sizes also possible, making it possible to measure transcriptome-scale expression across large continuous tissue areas. The high spatial resolution of 500 nm enables both cellular and subcellular-level analyses.
