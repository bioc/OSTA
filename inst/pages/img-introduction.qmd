...
Aligning spots along the image z-stack then allows individual targets to be identified based on a predefined barcoding scheme.
...
- In addition, the raw data typically include immunofluorescence signals (acquired as images in TIFF or ZARR format) from a nucleus stain (e.g., DNA or DAPI) and optionally cell surface stains (e.g., CD3, PanCK, ...). These signals are used for cell (and/or nucleus) segmentation, typically done with deep learning methods such as CellPose.
- A final preprocessing step assigns transcripts to segmented regions (cells and optionally nuclei) and counts how many transcripts per gene are present in each cell.
