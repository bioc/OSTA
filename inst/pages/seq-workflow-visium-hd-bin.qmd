# Workflow: Visium HD (binned) {#sec-seq-workflow-visium-hd-bin}
...
Visium HD is a next-generation spatial transcriptomics technology developed by 10x Genomics, designed to provide single-cell resolution spatial gene expression data across entire tissue sections. Visium HD advanced the platform's resolution from 55 µm spots to bins of subcellular resolution (2 µm). In this workflow, we demonstrate common analysis steps with a Visium HD dataset on colorectal cancer [@deOliveira2025-high-def].
...
In this demo, we perform deconvolution on 16 µm bins and compare the concordance with results on 8 µm bins, provided by 10x Genomics. See @sec-seq-deconvolution for a more focused discussion of deconvolution methods. [Here, we only demonstrate the analysis for patient 2 (P2). Other patients, such as P1 and P5, also have public Chromium, Visium HD, and Xenium data available for the CRC group (see @sec-bkg-example-datasets). It would be interesting to investigate the differences in malignant cells/bins (e.g., DEGs, pathways) between patients across different platforms.]{.aside}
...
unzip(pa, exdir=td)
# read 8 µm bins into 'SpatialExperiment'
vhd8 <- TENxVisiumHD(
    spacerangerOut=td, 
    processing="filtered",
    format="h5", 
...
These data come with bin-level annotations from deconvolution estimates by `RCTD` [@Cable2022-RCTD], implemented in `r BiocStyle::Biocpkg("spacexr")`. Specifically, two sets of labels are available that correspond to the most and second most frequent of 38 cell types per bin, respectively:
...
Another resolution of Visium HD with 16 µm bins can be read in as follows:

```{r load-vhd16, message=FALSE}
# read 16 µm bins into 'SpatialExperiment'
vhd16 <- TENxVisiumHD(
    spacerangerOut=td,
    processing="filtered",
    format="h5",
...
vhd8r <- .rng(vhd8)
# vhd16r <- .rng(vhd16) 
# use 8 µm bounding boxes to subset SPEs at both resolutions;
# it is similar to the 16 µm box range
.box <- \(roi, lty, lwd, col) {
    geom_rect(
...
First, we identify highly variable genes (HVGs) in the target area: [Note that spatially variable genes (SVGs) could be used instead; see @sec-seq-intermediate-processing.]{.aside}
...
As detailed in @sec-ind-clustering, `r BiocStyle::Biocpkg("Banksy")` [@Singhal2024-BANKSY] utilizes a pair of spatial kernels to capture gene expression variation, followed by dimension reduction and graph-based clustering to identify spatial domains.
...
Next, we can score marker genes for each cluster; c.f. @sec-ind-feature-set-signatures. We also compute the cluster-wise mean expression of selected markers:
...
We can see that myeloid cells are commonly colocalized with tumor and fibroblast cells; most doublets are homotypic, i.e., they are (mostly) composed of one low-resolution type.

::: {.callout-note title="Subset 8 µm bins within black bounding box" collapse="true"}
...
Weights inferred by `RCTD` correspond to proportions of cell types, such that they sum to 1 for a given observation (except for rejected ones, which sum to 0):
...
From the above heatmap, clusters 5 and 7 are mostly immune cells; cluster 9 maps to endothelial; clusters 1 and 13 correspond to intestinal epithelial and smooth muscle cells, respectively; fibroblasts map to cluster 10; tumor spans clusters 3, 11 and 12.

## Neighborhood analysis

Here, we want to quantify the abundance between *T cells*, *B cells*, or *Myeloid* bins to *Fibroblast* or *Tumor* bins in 8 µm zoomed region. We use `getAbundances()` from `r BiocStyle::Biocpkg("Statial")` [@Ameen2024-Kontextual] to calculate, for each bin, the abundance of other cell types within a radius of 200 px; the resulting (bins $\times$ types) matrix is stored as `reducedDim` slot `"abundances"`.

```{r Statial-run, message=FALSE, warning=FALSE}
...
[Please see the following chapter for cell-level Visium HD data analysis.]{.aside}
...
- [Sanofi's ENACT pipeline](https://github.com/Sanofi-Public/enact-pipeline) for cell segmentation, bin-to-cell assignment, and cell type inference for Visium HD data [@Kamel2025-ENACT].
